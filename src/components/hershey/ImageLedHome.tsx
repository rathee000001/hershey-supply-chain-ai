...
function FactoryStory({data}:{data:EnrichedArtifacts}){
 ...
 <div className="ih-inline-detail ih-stage-detail" style={{"--subject-color":current.color} as CSSProperties} aria-live="polite"><h3><current.Icon size={25}/>{current.label}</h3>
...
function AboutStory({data}:{data:EnrichedArtifacts}){return <section data-home-section className="ih-story-card" id="about-the-study"><div className="ih-card-heading"><span>03 / ABOUT THIS STUDY</span><h2>A real product.<br/>A public-evidence study.</h2><p>See the scope, purpose and limits together.</p></div><div className="ih-about-product"><Image src="/home-art/product.png" width={420} height={280} alt="Illustration of a partly unwrapped milk chocolate bar"/></div><div className="ih-inline-detail"><h3><Package size={24}/>Product scope</h3><p>{data.manifest.unit}</p><h3><GraduationCap size={25}/>Academic context</h3><p>An independent project by Praveen Rathee, connecting ingredient research, supply-chain context and benchmark cost calculations.</p><h3><ShieldCheck size={24}/>What this does not claim</h3><p>No exact supplier allocation, proprietary cost disclosure or tracked delivery route is inferred from a visual connection.</p><div className="ih-about-links"><Link href="/sources">Explore sources <ArrowUpRight size={16}/></Link><Link href="/methodology">How the study works <ArrowUpRight size={16}/></Link></div></div></section>}
