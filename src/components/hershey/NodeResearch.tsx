...
const sourceArt=(packet=""):StoryObjectName=>/sugar/.test(packet)?"sugar":/cocoa/.test(packet)?"cocoa":/dairy/.test(packet)?"dairy":/retail/.test(packet)?"retail":/logistic/.test(packet)?"truck":/packaging/.test(packet)?"packaging":"report";
...
export default function NodeResearch({data,nodeId,evidenceIds}:{data:EnrichedArtifacts;nodeId?:string;evidenceIds?:string[]}){
 ...
 {reading&&selected?<><button className="hc-pill" onClick={()=>setReading(false)}>← Back to related documents</button><div className="story-question-heading"><SubjectIllustration name={sourceArt(entry?.packet)} size={54}/><h3>{documentTitle(selected.name)}</h3></div><p>{scopeExplanation(entry?.safe_scope)}</p><ResearchDocumentReader fileName={selected.name}/></>:<><div className="story-record-list">{documents.slice(page*pageSize,page*pageSize+pageSize).map(document=><button className="research-story-card" key={document.name} aria-pressed={selected?.name===document.name} onClick={()=>{setFile(document.name);setReading(true)}}><span className="research-story-title"><SubjectIllustration name={sourceArt(data.evidence[document.records[0]]?.packet)} size={48}/><strong>{documentTitle(document.name)}</strong></span><p>{scopeExplanation(data.evidence[document.records[0]]?.safe_scope)}</p><small>Read the original here → · {document.records.length} linked research {document.records.length===1?"record":"records"}</small></button>)}</div>{!documents.length&&<p>No directly linked approved documents are recorded for this subject. The related-research view may provide background, but is not direct evidence of this modeled step.</p>}{documents.length>pageSize&&<div className="story-mini-actions"><button disabled={page===0} onClick={()=>setPage(value=>value-1)}>Previous</button><span>{page+1} / {Math.ceil(documents.length/pageSize)}</span><button disabled={(page+1)*pageSize>=documents.length} onClick={()=>setPage(value=>value+1)}>Next</button></div>}{entry&&<p className="ih-fine">{scopeExplanation(entry.safe_scope)}</p>}</>}
 </div>;
}
